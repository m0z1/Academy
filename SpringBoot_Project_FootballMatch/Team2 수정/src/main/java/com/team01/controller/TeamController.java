...
public class TeamController {
	...
	@ResponseBody
	public String team_insert(@RequestBody Team team) {
		
		SessionUser sessionUser = (SessionUser) httpSession.getAttribute("user");
		
		team.setTeamManager(sessionUser.getId());
	...
	public String team_view(@PathVariable Long teamId, Model model, Comment comment) {
	...
	@ResponseBody
	public String teamAd_insert(@RequestBody Board board) {
		board.setBoardWhere("team");
		SessionUser sessionUser = (SessionUser) httpSession.getAttribute("user");
		User user = userService.user_view(sessionUser.getEmail()).get(); 
		boardService.teamAd_insert(board, user);
		return "success";
	}
	
	@GetMapping("/team/team_join/{teamId}") // 팀 가입
	public String team_join(@PathVariable Long teamId) { //
		SessionUser sessionUser = (SessionUser) httpSession.getAttribute("user");
		User user = userService.user_view(sessionUser.getEmail()).get(); 
		userService.teamjoin(user, teamService.team_view(teamId));
		return "redirect:/";
	}
	...
	public String team_out(@PathVariable Long teamId) {
		
		SessionUser sessionUser = (SessionUser) httpSession.getAttribute("user");
		User user = userService.user_view(sessionUser.getEmail()).get(); 
		userService.teamout(user, teamService.team_view(teamId));
		return "redirect:/";
	}
