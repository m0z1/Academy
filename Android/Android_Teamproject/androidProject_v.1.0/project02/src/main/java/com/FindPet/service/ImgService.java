...
public class ImgService {
	...
	public void updateImgFile(Long imgNum, MultipartFile imgFile) throws Exception {
		if (!imgFile.isEmpty()) {
			ImgFile savedFieldImg = imgRepository.findById(imgNum).orElseThrow(EntityNotFoundException::new);

			// 기존 이미지 파일 삭제
			if (!savedFieldImg.getImgSaveName().isEmpty()) {
				fileService.deleteFile(imgLocation + "/" + savedFieldImg.getImgSaveName());
			}
